...
class Maquina:
    ...
    def __init__(self, ENA, IN1, IN2, ENB, IN3, IN4, # Motor
        ...
        self.angulo_global = 0.0
        self.distancia_global = -1 # Última distancia válida medida por el sensor activo
        self.estado_movimiento_global = "detenido" # "detenido" o "moviendo"
...
#                 debug_timer_move = time.ticks_ms() # Resetear timer de debug
        ...
        while True:
            ...
            if time.ticks_diff(ahora_task, self.last_publish_time_estado) > self.publish_interval_estado_ms:
                payload = {
                    ...
                    "distancia_sensor": self.distancia_global, # Ya es el último valor válido
                    "estado_movimiento": self.estado_movimiento_global,
    ...
    def _leer_sensor_promedio(self, sensor_obj, num_lecturas):# revisado
        ...
        if lecturas_validas_count == 0: # Si no hay lecturas válidas, retornar -1
            ...
            # No actualizamos self.distancia_global, para que mantenga el último valor válido.
            # self.debug_serial("[Sensor Error] Ninguna lectura válida en promedio. distancia_global no actualizada.")
            return -1 # Indicar error
        else:
            promedio = suma_lecturas // lecturas_validas_count # División entera
            self.distancia_global = promedio # Actualizar la variable global con el nuevo promedio válido
            # self.debug_serial(f"[Sensor Info] Promedio de {lecturas_validas_count} lecturas: {promedio} mm. distancia_global actualizada.") # Opcional
            return promedio
